...
class ImageProcessorApp:
    ...
    def create_widgets(self):
        ...
        # Input folder
        tk.Label(self.root, text="Input Folder:").grid(row=0, column=0, sticky="e", **padding_options)
        ...
        # Output folder
        tk.Label(self.root, text="Output Folder:").grid(row=1, column=0, sticky="e", **padding_options)
        ...
        tk.Radiobutton(self.root, text="Resize", variable=self.action_var, value="resize").grid(row=2, column=1, sticky="w")
        tk.Radiobutton(self.root, text="Crop", variable=self.action_var, value="crop").grid(row=2, column=2, sticky="w")

        # Target dimensions
        tk.Label(self.root, text="Target Size (Height x Width):").grid(row=3, column=0, sticky="w", **padding_options)
        ...
        tk.Label(self.root, text="x").grid(row=3, column=1, padx=(100, 5), pady=5, sticky="w")
        ...
        self.auto_rename_cb = tk.Checkbutton(self.root, text="Auto rename", variable=self.rename_var)
        self.auto_rename_cb.grid(row=4, column=2, sticky="w", padx=5, pady=10)

        # Detect Face checkbox
        self.detect_face_var = tk.BooleanVar()
        self.detect_face_check = tk.Checkbutton(self.root, text="Detect Face", variable=self.detect_face_var)
        ...
        self.flip_check = tk.Checkbutton(self.root, text="Flip Image", variable=self.flip_var)
        self.flip_check.grid(row=5, column=1, sticky="w", padx=10, pady=5)

        tk.Label(self.root, text="Flip Axis (0: Vertical, 1: Horizontal):").grid(row=5, column=2, sticky="w", **padding_options)
        ...
        self.rotate_check = tk.Checkbutton(self.root, text="Rotate Image", variable=self.rotate_var)
        self.rotate_check.grid(row=6, column=1, sticky="w", padx=10, pady=5)

        tk.Label(self.root, text="Rotate Angle (degrees):").grid(row=6, column=2, sticky="w", **padding_options)
        ...
        self.noise_check = tk.Checkbutton(self.root, text="Add Noise", variable=self.noise_var)
        self.noise_check.grid(row=7, column=1, sticky="w", padx=10, pady=5)

        tk.Label(self.root, text="Noise Level (0-100):").grid(row=7, column=2, sticky="w", **padding_options)
        ...
        # Process button
        self.process_button = tk.Button(self.root, text="Process Images", command=self.process_images, bg="blue", fg="white", font=('Arial', 12, 'bold'))
        self.process_button.grid(row=8, column=1, columnspan=1, pady=20)

        # Cancel button
        self.cancel_button = tk.Button(self.root, text="Cancel", command=self.cancel_processing, bg="red", fg="white", font=('Arial', 12, 'bold'))
        ...
        self.status_label.grid(row=10, column=0, columnspan=3, sticky="w", padx=10, pady=5)    
    ...
    def process_images(self):
        ...
        try:
            ...
            messagebox.showerror("Invalid Input", "Please enter valid numeric values for dimensions.")
            return
        ...
        try:
            ...
            messagebox.showerror("Invalid Input", "Please enter valid numeric values for flip axis, rotate angle, and noise level.")
            return
        ...
        if not image_files:
            messagebox.showerror("No Images", "No image files found in the input directory.")
            return
        ...
        threading.Thread(target=self.process_images_thread, args=(
            input_path, output_path, width, height, action, rename_files, detect_face, flip_image, flip_axis, rotate_image, rotate_angle, add_noise, noise_level, image_files
        ), daemon=True).start()
    ...
    def process_images_thread(self, input_path, output_path, width, height, action, rename_files, detect_face, flip_image, flip_axis, rotate_image, rotate_angle, add_noise, noise_level, image_files):
        ...
        for i, file in enumerate(image_files):
            ...
            result = self.processor.process_image(image_path, output_path, width, height, action, rename_files, detect_face, flip_image, flip_axis, rotate_image, rotate_angle, add_noise, noise_level)
